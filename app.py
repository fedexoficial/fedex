...
STATUS_META = {
    "label-created": {"label_key": "status.label-created", "default_label": "Etiqueta criada", "progress": 10},
    "picked-up": {"label_key": "status.picked-up", "default_label": "Coletado", "progress": 25},
    "in-transit": {"label_key": "status.in-transit", "default_label": "Em trânsito", "progress": 55},
    "arrived-hub": {"label_key": "status.arrived-hub", "default_label": "Chegou ao hub", "progress": 72},
    "out-for-delivery": {"label_key": "status.out-for-delivery", "default_label": "Saiu para entrega", "progress": 90},
    "delivered": {"label_key": "status.delivered", "default_label": "Entregue", "progress": 100},
    "exception": {"label_key": "status.exception", "default_label": "Exceção na entrega", "progress": 65},
}
...
TRANSLATIONS = {
    ...
    "pt": {
        ...
        "btn.admin_portal": "Portal Admin",
        "footer.copy": "© 2026 FedEx Logistics",
        "footer.desc": "Operações de envio, rastreamento e entrega",
        ...
        "status.exception": "Exceção na entrega",
    },
    "ar": {
        "nav.home": "الرئيسية",
        "nav.ship": "اشحن الآن",
...
def generate_tracking_id() -> str:
    ...
    for _ in range(20):
        ...
        if not tracking_id_exists(tracking_id):
            return tracking_id
    raise RuntimeError("Não foi possível gerar um ID de rastreamento único.")
    ...
    if DATA_BACKEND == "firestore":
        ...
        db.execute(
            ...
            (
                ...
                "label-created",
                payload["origin_label"],
                "Envio registrado e etiqueta gerada.",
                now,
            ),
        )
        db.commit()
...
def extract_payload(form) -> dict:
    payload = {
        ...
        "origin_lat": parse_float(form.get("origin_lat", ""), "Latitude de origem"),
        "origin_lng": parse_float(form.get("origin_lng", ""), "Longitude de origem"),
        ...
        "destination_lat": parse_float(form.get("destination_lat", ""), "Latitude de destino"),
        "destination_lng": parse_float(form.get("destination_lng", ""), "Longitude de destino"),
        "package_description": form.get("package_description", "").strip(),
        "weight_kg": parse_float(form.get("weight_kg", ""), "Peso"),
...
def track_lookup():
    ...
    if not tracking_id:
        flash("O número de rastreamento é obrigatório.", "error")
...
def ship_submit():
    try:
        ...
        flash(f"Envio criado. ID de rastreamento: {tracking_id}", "success")
        return redirect(url_for("track_page", tracking_id=tracking_id))
...
def admin_create_shipment():
    try:
        ...
        flash(f"Envio criado pela equipe de operações. ID de rastreamento: {tracking_id}", "success")
    except ValueError as exc:
        flash(str(exc), "error")
    except RuntimeError as exc:
        flash(str(exc), "error")
...
def admin_update_status():
    ...
    if new_status not in STATUS_META:
        flash("Status inválido selecionado.", "error")
        return redirect(url_for("admin_dashboard"))

    shipment, _ = load_shipment_with_events(tracking_id)
    if not shipment:
        flash("ID de rastreamento não encontrado.", "error")
        return redirect(url_for("admin_dashboard"))

    if not location_label:
        location_label = shipment["destination_label"] if new_status == "delivered" else shipment["origin_label"]
    if not note:
        note = f"Status alterado para {status_label(new_status)} pela equipe de operações."
    ...
    flash(f"{tracking_id} atualizado para {status_label(new_status)}.", "success")
    return redirect(url_for("admin_dashboard"))
